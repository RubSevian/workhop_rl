...
import argparse, hashlib, shutil, struct, subprocess, tempfile, re
...
def add_stock_go2_visuals(root):
 stock = Path(__file__).resolve().parents[1] / 'unitree_robots/go2/go2.xml'
 stock_root = ET.parse(stock).getroot(); asset = root.find('asset')
 for node in stock_root.find('asset'):
  if node.tag in ('material', 'mesh'):
 ...
 for stock_body in stock_root.findall('.//body'):
  ...
  for geom in stock_body.findall('geom'):
   if geom.get('mesh'):
    visual = ET.fromstring(ET.tostring(geom)); visual.attrib.pop('class', None)
    visual.set('contype', '0'); visual.set('conaffinity', '0'); visual.set('group', '2'); target.append(visual)
...
def main():
 ...
 # Use the original Unitree terrain scene; only swap the robot include.
 terrain = (Path(__file__).resolve().parents[1] / 'unitree_robots/go2/scene_terrain.xml').read_text()
 terrain = terrain.replace('file="go2.xml"', 'file="go2_rars01.xml"')
 terrain = re.sub(r'<hfield[^>]*/>', '', terrain)
 terrain = re.sub(r'<geom type="hfield"[^>]*/>', '', terrain)
 (out/'scene.xml').write_text(terrain)

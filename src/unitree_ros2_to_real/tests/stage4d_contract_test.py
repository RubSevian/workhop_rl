...
class Stage4DContract(unittest.TestCase):
 def test_launch_chain_and_ownership(self):
  ...
  for x in ('terrain_analysis','terrain_analysis_ext','far_planner','localPlanner','pathFollower','stage4d_readiness.py','stage4d_full_navigation_evaluator.py','stage4d_rviz_robot.py','/cloud_registered','/point_lio/path','camera_init'): self.assertIn(x,s)
  self.assertNotIn('stage4c_pointlio_evaluator.py',s)
  self.assertIn('/stage4d/robot_model',(ROOT/'scripts/stage4d_rviz_robot.py').read_text())

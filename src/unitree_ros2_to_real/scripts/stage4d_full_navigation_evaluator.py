...
class Evaluator(Node):
 def __init__(self, report):
  ...
  self.create_subscription(PointStamped,'/goal_point',lambda m:self.set('goal',{'x':m.point.x,'y':m.point.y,'frame':m.header.frame_id}),10)
  for typ,topic,key in [(Odometry,'/state_estimation','odom'),(Odometry,'/sim/ground_truth_odom','gt'),(Path,'/path','path'),(PointStamped,'/way_point','waypoint'),(Twist,'/cmd_vel','cmd'),(Bool,'/navigation_active','navigation_active'),(Bool,'/far_reach_goal_status','far_reach_goal'),(Float32,'/planning_time','planning_time'),(Float32,'/runtime','runtime'),(Bool,'/stage4d/navigation_ready','navigation_ready')]: self.create_subscription(typ,topic,lambda m,k=key:self.set(k,m),10)
 ...
 def set(self,k,m):
  ...
  elif k=='cmd': v={'vx':m.linear.x,'vy':m.linear.y,'wz':m.angular.z}
...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--report',default='/tmp/stage4d_full_navigation_report.json'); a=p.parse_args(); rclpy.init(); n=Evaluator(a.report); rclpy.spin(n)

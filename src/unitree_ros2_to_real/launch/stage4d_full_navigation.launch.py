...
def generate_launch_description():
    ...
    return LaunchDescription([
      ...
      Node(package='unitree_legged_real', executable='pointlio_state_adapter.py', arguments=['--frame','camera_init'], output='screen'),
      Node(package='terrain_analysis', executable='terrainAnalysis', output='screen', remappings=[('/registered_scan','/cloud_registered')], parameters=[{'vehicleHeight':1.5,'clearingDis':8.0,'noDataObstacle':True}]),
      Node(package='terrain_analysis_ext', executable='terrainAnalysisExt', output='screen', remappings=[('/registered_scan','/cloud_registered')], parameters=[{'vehicleHeight':1.5,'clearingDis':30.0}]),
      ...
      Node(package='local_planner', executable='localPlanner', name='localPlanner', output='screen', parameters=[{'is_real_robot':False,'autonomyMode':True,'autonomySpeed':0.35,'maxSpeed':0.35,'allowStaticPath':False,'pathTimeoutSec':0.5,'odomTimeoutSec':0.5,'goalCloseDis':0.3}], remappings=[('/registered_scan','/cloud_registered')]),
